...
class AppPreferences {
  ...
  static Future<bool> setValue<T>(PreferenceItem<T> item, T? value) async {
    ...
    if (isNullable && value == null) {
      //null을 세팅한다는 것은 값을 지운다는 의미로 해석. 필요에 따라 변경해서 쓰시면 되			
			return _prefs.remove(item.key);
    }

    if (isNullable) {
      switch (T.toString()) {
        case "int?":						return _prefs.setInt(key, value as int);
        case "String?":					return _prefs.setString(key, value as String);
        case "double?":					return _prefs.setDouble(key, value as double);
        case "bool?":						return _prefs.setBool(key, value as bool);
        case "List<String>?":		return _prefs.setStringList(key, value as List<String>);
        case "DateTime?":				return _prefs.setString(key, (value as DateTime).toIso8601String());
        default: 								return (value is Enum) 
  ...
  static T getValue<T>(PreferenceItem<T> item) {
    final String key = getPrefKey(item);
    switch (T) {
      case int		:	return _prefs.getInt(key) as T? ?? item.defaultValue;
      case String	:	return _prefs.getString(key) as T? ?? item.defaultValue;
      case double	:	return _prefs.getDouble(key) as T? ?? item.defaultValue;
      case bool		:	return _prefs.getBool(key) as T? ?? item.defaultValue;
      case const (List<String>):			return _prefs.getStringList(key) as T? ?? item.defaultValue;
      default:
  ...
  static T? transform<T>(Type t, String? value) {
    ...
    if (isNullableType) {
      switch (t.toString()) {
        case "CustomTheme?":
          return ThemeType.values.asNameMap()[value] as T?;
        case "DateTime?":
