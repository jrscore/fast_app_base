...
class FbServerException implements Exception {
  ...
  final String code;
  FbServerException(this.code);


	String get message {
		switch (code) {
			case 'unknown':											return '알 수 없는 Firebase 오류가 발생했습니다. 다시 시도해 주세요.';
			case 'invalid-custom-token':				return '사용자 지정 토큰 형식이 잘못되었습니다. 사용자 지정 토큰을 확인해 주세요.';
			case 'custom-token-mismatch':				return '사용자 지정 토큰이 다른 대상에 해당됩니다.';
			case 'user-disabled':								return '사용자 계정이 비활성화되었습니다.';
			case 'user-not-found':							return '제공된 이메일 또는 UID에 해당하는 사용자를 찾을 수 없습니다.';
			case 'invalid-email':								return '제공된 이메일 주소가 유효하지 않습니다. 유효한 이메일을 입력해 주세요.';
			case 'email-already-in-use':				return '이메일 주소가 이미 등록되어 있습니다. 다른 이메일을 사용해 주세요.';
			case 'wrong-password':							return '잘못된 비밀번호입니다. 비밀번호를 확인하고 다시 시도해 주세요.';
			case 'weak-password':								return '비밀번호가 너무 약합니다. 더 강력한 비밀번호를 선택해 주세요.';
			case 'provider-already-linked':			return '계정이 이미 다른 제공자와 연결되어 있습니다.';
			...
			case 'captcha-check-failed':				return 'reCAPTCHA 응답이 유효하지 않습니다. 다시 시도해 주세요.';
			case 'app-not-authorized':					return '제공된 API 키로 Firebase 인증을 사용할 수 있는 권한이 이 앱에 없습니다.';
			...
			case 'requires-recent-login':				return '이 작업은 민감하며 최근 인증이 필요합니다. 다시 로그인해 주세요.';
			case 'quota-exceeded':							return '할당량을 초과했습니다. 나중에 다시 시도해 주세요.';
			case 'missing-iframe-start':				return '이메일 템플릿에서 iframe 시작 태그가 누락되었습니다.';
			case 'missing-iframe-end':					return '이메일 템플릿에서 iframe 종료 태그가 누락되었습니다.';
			case 'missing-iframe-src':					return '이메일 템플릿에서 iframe src 속성이 누락되었습니다.';
			...
			case 'missing-action-code':					return '액션 코드가 누락되었습니다. 유효한 액션 코드를 제공해 주세요.';
			...
			case 'expired-action-code':					return '액션 코드가 만료되었습니다. 새로운 액션 코드를 요청해 주세요.';
			case 'invalid-action-code':					return '액션 코드가 유효하지 않습니다. 코드를 확인하고 다시 시도해 주세요.';
			case 'credential-already-in-use':		return '이 자격증명은 이미 다른 사용자 계정과 연결되어 있습니다.';
			case 'account-exists-with-different-credential': return '같은 이메일을 가진 계정이 이미 존재하지만, 다른 로그인 자격증명이 사용되었습니다.';
			default:								return '예상치 못한 Firebase 오류가 발생했습니다. 다시 시도해 주세요.';
		}
	}
